...
export function VoiceInput({ onTextGenerated, className }: VoiceInputProps) {
  ...
  const playTone = (type: 'start' | 'end' | 'invocationException') => {
    ...
    try {
      ...
      mediaRecorderRef.current.onstop = async () => {
        ...
        if (chunksRef.current.length > 0) {
          ...
          const { data, error: invocationException } = await supabase.functions.invoke('transcribe-idea', { body: formData });
          if (invocationException || !data?.success) throw new Error("Fallo en transcripción");
  ...
  return (
    <div className={classNamesUtility("w-full", className)}>
      {status === 'idle' ? (
