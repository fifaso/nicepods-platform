...
// VERSIÓN: 2.4 (Aurora Shell - Loading Context Awareness)
...
import { ChevronLeft, ChevronRight, Loader2, FileText, Wand2, Compass, Zap, Sparkles } from "lucide-react";
...
export function LayoutShell({ children, onNext, onProduce, onDraft, onAnalyzeLocal, isGenerating, isSubmitting, progress }: LayoutShellProps) {
  const { currentFlowState, goBack } = useCreationContext();

  // [LÓGICA]: Si estamos en el Loader, ocultamos el Header y Footer del Shell
  // para dejar que DraftGenerationLoader tome el control total de la experiencia.
  ...
  return (
    <div className="fixed inset-0 flex flex-col bg-transparent overflow-hidden h-[100dvh]">
      ...
      <main className="flex-1 overflow-hidden flex flex-col items-center justify-center p-4">
        <div className={cn("w-full h-full flex flex-col transition-all duration-700", progress.isInitial ? "max-w-5xl" : "max-w-4xl")}>
