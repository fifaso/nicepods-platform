...
export function UsersTable({ users }: UserTableProps) {
  ...
  // Fallback si users es null/undefined
          ...
          {safeUsers.length === 0 ? (
             <TableRow>
                <TableCell colSpan={4} className="text-center py-8 text-slate-500">
                    No se encontraron usuarios o hubo un error de carga.
                </TableCell>
             </TableRow>
          ) : (
            safeUsers.map((user) => {
                // [LÓGICA DEFENSIVA]: Manejo robusto de la relación user_usage
                let usage = 0;
                
                if (user.user_usage) {
                    if (Array.isArray(user.user_usage) && user.user_usage.length > 0) {
                        usage = user.user_usage[0].podcasts_created_this_month || 0;
                    } else if (typeof user.user_usage === 'object' && !Array.isArray(user.user_usage)) {
                        // Supabase a veces devuelve objeto directo en relaciones 1:1
                        usage = (user.user_usage as any).podcasts_created_this_month || 0;
                    }
                }
