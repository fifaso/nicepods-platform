...
// VERSIÓN: 2.1 (Madrid Resonance - Physical Path Bypass)
...
/**
 * [EL FIX DEFINITIVO]
 * Importamos directamente desde el archivo compilado ESM de la librería.
 * Esto evita que Vercel intente leer el 'package.json' del root, que está roto para PNPM.
 */
// @ts-ignore
        ...
        {selectedMemory && (
          <Popup latitude={selectedMemory.lat} longitude={selectedMemory.lng} anchor="top" onClose={() => setSelectedMemory(null)} closeButton={false}>
            <div className="p-4 bg-zinc-950 border border-white/10 rounded-2xl shadow-2xl min-w-[220px]">
              ...
              <button className="w-full bg-primary text-white text-[10px] font-black py-3 rounded-xl mt-3 flex items-center justify-center gap-2 hover:brightness-110 active:scale-95 transition-all" onClick={() => window.location.href = `/podcast/${selectedMemory.id}`}>
                <Play className="w-3.5 h-3.5 fill-current" /> ESCUCHAR
              </button>
            </div>
          </Popup>
        )}
