...
// VERSIÓN DE PRODUCCIÓN FINAL: Resiliente a respuestas de API vacías.
...
if (!GOOGLE_CLIENT_EMAIL || !GOOGLE_PRIVATE_KEY_RAW || !GOOGLE_PROJECT_ID) {
  throw new Error("FATAL: Uno o más secretos de Google Cloud (CLIENT_EMAIL, PRIVATE_KEY, PROJECT_ID) no están configurados.");
}
...
const createVisualPrompt = (title, scriptText)=>{
...
serve(async (request)=>{
  ...
  try {
    ...
    if (!imageResponse.ok) {
      ...
      throw new Error(`API de Imagen (Vertex AI) falló con status ${imageResponse.status}: ${errorText}`);
    }
    const responseData = await imageResponse.json();
    // [INTERVENCIÓN QUIRÚRGICA] Verificación robusta para evitar el TypeError.
    const imageBase64 = responseData.predictions?.[0]?.bytesBase64Encoded;
    if (!imageBase64) {
      console.error("Respuesta de Vertex AI exitosa pero sin datos de imagen:", JSON.stringify(responseData, null, 2));
      throw new Error("La respuesta de la IA no contenía datos de imagen válidos.");
    }
    ...
    console.error(`Error en generate-cover-image para el job ${jobId}:`, error);
    return new Response(JSON.stringify({
      error: error.message
    }), {
      status: 500,
