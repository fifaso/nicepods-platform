...
// VERSIÓN DE PRODUCCIÓN FINAL (CON DEPENDENCIA CORREGIDA)
...
// Se reemplaza la URL de importación rota de `deno.land/x` por una versión
// robusta y compatible de la misma librería a través de `esm.sh`.
...
serve(async (request: Request) => {
  ...
  try {
    ...
    let audioDuration = 0;
    try {
      const metadata = await mm.parseBlob(new Blob([decodedAudio]));
    ...
    if (updateError) {
      throw new Error(`Fallo al actualizar el podcast con la URL y duración: ${updateError.message}`);
    }
