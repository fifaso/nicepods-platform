...
// VERSIÓN: 3.0 (The Intelligence Command Suite - High Fidelity & Space Optimized)
// Misión: Punto de entrada operativo. Gestiona el espacio mediante estados dinámicos y jerarquía visual de élite.
 ...
 * Cargamos el Mapa Panorámico y las Estanterías de Audio de forma diferida.
 * El skeleton (loading) respeta las nuevas dimensiones ultra-compactas.
 */
...
/**
 * sanitizePodcasts
 * Asegura que los datos provenientes de RPC mantengan la integridad estructural para el UI.
 */
...
export default async function DashboardPage() {
  ...
  // 1. PROTOCOLO DE IDENTIDAD (Server-Side)
  ...
  return (
    <main className="container mx-auto max-w-screen-xl min-h-screen px-4 lg:px-6">
      <div className="grid grid-cols-1 lg:grid-cols-4 lg:gap-12 xl:gap-16 pt-6 pb-24">
        ...
        {/* COLUMNA PRINCIPAL (Malla Operativa) */}
        <div className="lg:col-span-3 space-y-10">
          
          {/* SECCIÓN I: COMMAND BAR (Header Compacto)
              Diseñado para que el saludo y la búsqueda compartan el eje visual.
          */}
          <header className="relative w-full flex items-center justify-between min-h-[80px] animate-in fade-in slide-in-from-top-4 duration-700">
            ...
            {/* Lado Derecho: Buscador Expansivo (Integrado vía DiscoveryHub)
                En móvil y desktop, este componente cubrirá el saludo al activarse.
            */}
          ...
          {/* SECCIÓN II: PORTAL DE RESONANCIA PANORÁMICO
              Reducido en altura para ganar espacio vertical y mostrar contenido "Above the fold".
          */}
          <section className="relative group rounded-[2.5rem] md:rounded-[3.5rem] overflow-hidden border border-white/5 shadow-2xl transition-all duration-700 hover:border-primary/20 bg-zinc-950">
            ...
            {/* Altura optimizada: 140px en móvil, 180px en desktop */}
            <div className="h-[140px] md:h-[180px] w-full">
              <MapPreviewFrame />
            </div>

            {/* Sombra de Integración */}
            <div className="absolute inset-0 bg-gradient-to-t from-background via-transparent to-transparent pointer-events-none z-10" />
          </section>

          {/* SECCIÓN III: MALLA DE CONOCIMIENTO (Feed de Valor) */}
          <div className="space-y-16">
            
            {/* Categorías de Universos Semánticos (Carrusel compacto) */}
            ...
            {/* Estantes de Podcasts (Densidad Aumentada) */}
        ...
        {/* COLUMNA LATERAL (Insights & Soberanía) */}
        <aside className="hidden lg:block lg:col-span-1">
          <div className="sticky top-[7.5rem] space-y-8">
            ...
            {/* Panel de Gráficos (Visualización de Datos) */}
            <InsightPanel resonanceProfile={resonanceProfile} />
            
            {/* Branding y Versión (Sutil) */}
            <div className="p-8 text-center bg-white/[0.02] rounded-[2.5rem] border border-white/5 flex flex-col items-center space-y-4">
