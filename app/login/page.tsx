// app/login/page.tsx
// VERSIÓN: 3.0 (Smart Redirect on Load - Previene formulario si ya hay sesión)
...
export default function LoginPage() {
  ...
  // Iniciamos isLoading en true para evitar "falso render" del formulario mientras verificamos sesión
  const [isLoading, setIsLoading] = useState(true) 
  
  const { supabase, user } = useAuth(); // Obtenemos 'user' del hook
  ...
  // [NUEVO] Efecto de "Portero": Si ya estás dentro, te saco de aquí.
  useEffect(() => {
    const checkExistingSession = async () => {
      // 1. Verificamos si hay usuario activo (el hook useAuth ya nos lo da, pero confirmamos rol)
      const { data: { session } } = await supabase.auth.getSession();
      
      if (session?.user) {
        // Consultamos rol para redirigir correctamente
        ...
        if (profile?.role === 'admin') {
            router.replace('/admin'); // replace evita que puedan volver atrás con el botón back
        } else {
            router.replace(redirectUrl);
        }
      } else {
        // Solo si NO hay sesión, mostramos el formulario
        setIsLoading(false);
      }
    };
  ...
  const handleEmailLogin = async (e: React.FormEvent) => {
    ...
    // Verificación de rol post-login (redundante pero necesaria para el flujo manual)
    try {
        const { data: profile } = await supabase
            .from('profiles')
            .select('role')
  ...
  // Renderizado condicional: Si estamos verificando sesión, mostramos spinner limpio
  if (isLoading && !email) { // !email asegura que no oculte el form si el usuario ya empezó a escribir tras un error
      return (
  ...
  return (
    <div className="flex min-h-screen items-center justify-center p-4 bg-gradient-to-br from-purple-500/10 via-background to-blue-500/10 dark:from-purple-900/20 dark:via-background dark:to-blue-900/20">
      <Card className="w-full max-w-md backdrop-blur-lg bg-card/60 dark:bg-card/40 border-muted/30 shadow-2xl animate-in fade-in zoom-in-95 duration-300">
